...
function createFullscreenButton(store, rootContainer, mainUIRow) {
  ...
  if (fullScreenMethods) {
    ...
    fullscreenButton.innerHTML = `<input id="${store.id}-toggleFullscreenButton" type="checkbox" class="${style.toggleInput}"><label itk-vtk-tooltip itk-vtk-tooltip-top-annotation itk-vtk-tooltip-content="Fullscreen[f]" class="${style.fullscreenButton} ${style.toggleButton}" for="${store.id}-toggleFullscreenButton">${fullscreenIcon}</label>`
    ...
    reaction(
      () => store.mainUI.fullscreenEnabled,
      fullscreenEnabled => {
