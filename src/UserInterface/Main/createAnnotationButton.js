...
function createAnnotationButton(store, mainUIRow) {
  ...
  annotationButton.innerHTML = `<input id="${store.id}-toggleAnnotationsButton" type="checkbox" class="${style.toggleInput}" checked><label itk-vtk-tooltip itk-vtk-tooltip-top-annotation itk-vtk-tooltip-content="Annotations" class="${style.annotationButton} ${style.toggleButton}" for="${store.id}-toggleAnnotationsButton">${annotationIcon}</label>`
