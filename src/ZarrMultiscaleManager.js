...
class ZarrMultiscaleManager extends MultiscaleManager {
  ...
  static async parseMetadata(url) {
    ...
    const multiscaleLevels = zmetadata['.zattrs']._MULTISCALE_LEVELS !== undefined ?
      zmetadata['.zattrs']._MULTISCALE_LEVELS :
      [""];
    let pixelArrayName = zmetadata['.zattrs']._SPATIAL_IMAGE !== undefined ?
      zmetadata['.zattrs']._SPATIAL_IMAGE :
      null;
    if (pixelArrayName !== null) {
      const pixelArrayAttrs = multiscaleLevels[0] === "" ? `${pixelArrayName}/.zattrs`: `${multiscaleLevels[0]}/${pixelArrayName}/.zattrs`;
      ...
      const pixelArrayMeta = multiscaleLevels[0] === "" ? `${pixelArrayName}/.zarray`: `${multiscaleLevels[0]}/${pixelArrayName}/.zarray}`;
      bottomMeta.pixelArrayMetadata = zmetadata[pixelArrayMeta];
      bottomMeta.pixelArrayName = pixelArrayName;
      bottomMeta.pixelArrayUrl = multiscaleLevels[0] === "" ? `${url}/${bottomMeta.pixelArrayName}/`: `${url}/${multiscaleLevels[0]}/${bottomMeta.pixelArrayName}/`;

      const coordPrefix = multiscaleLevels[0] === "" ? "": `${multiscaleLevels[0]}/`;
    ...
    bottomMeta.coords.forEach((value, key) => {
      const coordPrefix = multiscaleLevels[0] === "" ? "": `${multiscaleLevels[0]}/`;
      bottomMeta.coords.set(key, decompressCoordPromise(url, zmetadata, `${coordPrefix}${key}`));
    })
    ...
    async function levelMetadata(levelPath) {
      ...
      meta.coords.forEach((value, key) => {
        meta.coords.set(key, decompressCoordPromise(url, zmetadata, `${levelPath}/${key}`));
      })
      // We need this earlier and it is small -- resolve it now.
