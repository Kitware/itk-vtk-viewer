...
function createUseShadowToggle(store, uiContainer) {
  ...
  useShadowButton.innerHTML = `<input id="${store.id}-toggleShadowButton" type="checkbox" class="${style.toggleInput}" checked><label itk-vtk-tooltip itk-vtk-tooltip-top-annotation itk-vtk-tooltip-content="Use shadow" class="${style.shadowButton} ${style.toggleButton}" for="${store.id}-toggleShadowButton">${shadowIcon}</label>`
