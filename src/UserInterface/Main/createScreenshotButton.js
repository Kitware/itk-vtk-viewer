...
function createScreenshotButton(store, mainUIRow) {
  ...
  screenshotButton.innerHTML = `<input id="${store.d}-screenshotButton" type="checkbox" checked class="${
      style.toggleInput
    }"><label itk-vtk-tooltip itk-vtk-tooltip-top-annotation itk-vtk-tooltip-content="Screenshot" class="${style.screenshotButton} ${style.toggleButton}" for="${store.id}-screenshotButton">${screenshotIcon}</label>`;
